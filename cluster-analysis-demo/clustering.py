...
from utils.src.dir_support import reset_directory
...
def clustering(dir_basket, D, list_basket_paths, number_of_clusters):
    ...
    print(f'\nInitial contents of each cluster:')
    for n in range(n_objects):
    ...
    for k in range(n_objects_1):
        ...
        print(f'Cluster merging:\t\t I = {i}\t\t J = {j}\t\t minD = {minD:.5f}')

        print(f'\nN[] - number of objects in each cluster::')
        print(f'\t {N}')
